...
def insert_locations(cur) -> list[int]:
    rows = [
        (city, state, country, lat, lon) for city, state, country, lat, lon in CITIES
    ]
...
def insert_promo_codes(cur) -> list[dict]:
    ...
    for code, dtype, val, _ in PROMO_CATALOG:
        valid_from = rand_dt(
            START_DATE - timedelta(days=60), START_DATE + timedelta(days=60)
        )
        valid_until = valid_from + timedelta(days=random.randint(90, 365))
        rows.append(
            (code, dtype, val, valid_from, valid_until, random.randint(200, 5000), True)
        )
    ...
    return [
        {
            "id": ids[i],
            "type": PROMO_CATALOG[i][1],
            "value": PROMO_CATALOG[i][2],
            "prob": PROMO_CATALOG[i][3],
        }
...
def insert_vehicle_assignments(
    cur, vehicle_ids: list[int], driver_ids: list[int]
) -> None:
    """
...
def _build_trip(
    driver_ids, passenger_ids, vehicle_ids, location_ids, pm_ids, promos
) -> tuple:
    ...
    tip = (
        round(base_fare * random.uniform(0.0, 0.25), 2)
        if status == "completed"
        else 0.00
    )
    ...
    if status == "completed" and random.random() < 0.20:
        ...
        discount = round(
            min(raw, base_fare), 2
        )  # satisfies chk_discount_not_exceed_base
...
def insert_trips(
    cur, driver_ids, passenger_ids, vehicle_ids, location_ids, pm_ids, promos
) -> list[tuple]:
    """
...
def insert_trip_cancellations(cur, trip_meta: list[tuple]) -> int:
    ...
    cancelled = [
        (tid, req_at) for tid, status, req_at in trip_meta if status == "cancelled"
    ]
    ...
    rows = [
        (
            tid,
            req_at
            + timedelta(minutes=random.randint(1, 15)),  # cancelled within 15 min
